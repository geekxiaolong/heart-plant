 ...
 * 用法（在 heart-plant 目录下）：EMAIL=776427024@qq.com PASSWORD=你的密码 node scripts/fix-user-plant-species.mjs
 * 环境变量：EMAIL, PASSWORD；API_BASE（可选，默认 Supabase 云函数）
 */
...
async function main() {
  if (!PASSWORD) {
    console.error("请设置环境变量 PASSWORD");
    process.exit(1);
  }
